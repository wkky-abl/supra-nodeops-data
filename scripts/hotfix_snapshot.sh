...
docker rmi asia-docker.pkg.dev/supra-devnet-misc/smr-moonshot-devnet/validator-node:v4.0.2
echo
...
echo "Running new docker image"
    ...
    docker run --name "supra_$ip_address" \
        ...
        --net=host \
        -itd  asia-docker.pkg.dev/supra-devnet-misc/smr-moonshot-devnet/validator-node:v4.1.3
echo
echo "New docker Container with image is created"

# Clean old database
sudo rm -rf ./supra_configs/ledger_storage ./supra_configs/smr_storage/* ./supra_configs/supra_node_logs
